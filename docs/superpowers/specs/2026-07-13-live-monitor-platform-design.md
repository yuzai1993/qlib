...
类型语义：`DEPOSIT / WITHDRAW / CORRECTION` = 外部出入金（日收益剔除）；`DIVIDEND / DIVIDEND_TAX / BONUS_SHARES` = 公司行为（计入收益）。手工入口 `scripts/record_cash_flow.py`；Web 新增「资金流水」页（`GET /api/cashflows`）。

### 12.3 分红/送股与红利税

规则查证（财税〔2015〕101号，2015-09-08 起）：持股 ≤1 月税负 20%、1 月~1 年 10%、>1 年免；派发时不预扣，**卖出时**由中国结算按先进先出计算、券商从资金账户补扣。

实现：`modules/corporate_actions.py`。report 阶段快照前调用——tushare `dividend` 接口按 `ex_date=当日` 查持仓股实施分红：现金红利按税前入账（`DIVIDEND`）+ 按 `fees.dividend_tax_rate`（默认 20%，本策略持有期约一周、几乎必按 20% 补扣）预提（`DIVIDEND_TAX`）；送转股 `apply_bonus_shares` 加股数摊薄成本（`BONUS_SHARES` 流水留痕）。dedup_key = `DIV_/DIVTAX_/BONUS_{code}_{date}`，重跑幂等。查询失败发 `CORP_ACTION_FAILED` WARN。与券商实扣差额人工 `CORRECTION` 校正。

### 12.4 快照口径变化

- `build_snapshot` 新增 `external_flow` / `fees` 参数：`daily_return = (total_value - external_flow) / prev_total - 1`；
- 累计收益改为按日收益**链式累乘**（出入金只改基数不计业绩），不再用 `total/first_total`；
- `daily_snapshot` 新增 `fees` / `external_flow` 列（ALTER TABLE 自动迁移），日报展示当日费用与出入金。

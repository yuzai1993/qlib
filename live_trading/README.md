...
| 执行时间 | 15:05 卖出，等待最多 4 分钟后买入，15:28 撤单，15:30 终结，15:31 快照 |
...
BUY 计划只携带 `target_value`，计划股数为 0。QMT 在卖单阶段后读取实际现金和官方收盘价，向下取整为整手并预留费用。SELL 计划携带账本股数。即使当天没有订单，也会发布 header-only 批次并得到空回执文件；`planned=0, terminal=0, missing=0` 是正常终态，不是漏跑。

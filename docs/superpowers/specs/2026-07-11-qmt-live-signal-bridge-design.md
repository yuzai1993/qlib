...
| T 09:25–09:35 | QMT | 消费当日 `batch`：**先提交全部卖单**；`quickTrade=2` 立即下单 |
| T 09:35–10:30 | QMT | 卖单全部终态（或超时 30 分钟）后提交买单；买前查可用资金，不足则整手缩量 |
| T 盘中 | QMT | 轮询委托/成交，增量写 `fills` |
| T 14:50 | QMT | 对仍未终态的委托执行撤单（`cancel`），标记 `EXPIRED`/`PARTIAL` |
| T 14:55 前 | QMT | **强制写 `fills_{batch_id}.done`**（收盘后 handlebar 不再触发，必须盘中兜底） |

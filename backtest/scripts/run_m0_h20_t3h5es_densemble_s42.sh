#!/bin/bash
# Phase M v1：相对 v3 换 B6-M DoubleEnsemble，先跑种子 42。不作晋升依据。
set -euo pipefail
cd /Users/yuxianqi/Project/qlib_exp
export MLFLOW_ALLOW_FILE_STORE=true
trap 'echo DENSEMBLE-V3-FAILED $(date "+%F %H:%M")' ERR
PY=/opt/anaconda3/envs/qlib/bin/python
CFG=backtest/configs/regime-adapt
OUT=backtest/result/eval_regime_ablation
LOG=backtest/result/regime_densemble_v3_s42.log
SESS=regimeadapt_m0h20_t3h5es_densemble_s42
mkdir -p "$OUT"

echo "===== DENSEMBLE-V3-START $(date '+%F %H:%M') =====" | tee -a "$LOG"
...
echo "EVAL-START densemble-v3 $(date '+%m-%d %H:%M')" | tee -a "$LOG"
$PY backtest/scripts/eval_ic_multi_pool.py --config "$CFG/eval_m0fast.yaml" \
    ...
    --regime-pools all \
    --output "$OUT/eval_densemble_s42_vs_v3.json" | tee -a "$LOG"
echo "EVAL-DONE densemble-v3 $(date '+%m-%d %H:%M')" | tee -a "$LOG"

$PY backtest/scripts/register_regime_ablation.py --spec densemble-v3 | tee -a "$LOG"
$PY backtest/scripts/build_regime_phase_m_detail_report.py --spec densemble-v3 | tee -a "$LOG"
$PY backtest/scripts/build_phase_m_v1_report.py | tee -a "$LOG"

echo "===== DENSEMBLE-V3-DONE $(date '+%F %H:%M') =====" | tee -a "$LOG"

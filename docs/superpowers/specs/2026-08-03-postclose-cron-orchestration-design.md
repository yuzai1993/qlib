...
整个流水线持有配置级 `postclose` 目录锁。锁存在时，21:30 发布和人工 catch-up 都必须失败关闭，避免读取正在改写的 Qlib provider。为消除两个任务同时启动的检查/加锁竞争，postclose 在持锁后检查 `publish` 锁，publisher 在取得 `publish` 锁后再次检查 `postclose` 锁；任一方向发现冲突都保守退出。异常退出由 trap 释放锁；若机器强制重启留下残锁，运维人员必须先确认没有任务运行再人工处理。

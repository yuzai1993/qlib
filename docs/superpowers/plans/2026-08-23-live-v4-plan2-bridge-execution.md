...
- 被抵销的两腿回执是 `status=SKIPPED` + `netted_qty>0` + `applied_qty=0`。与监控的冲突点**不是** `reject_rate`——`pipeline_monitor._REJECT_STATUS` 只含 `{REJECTED, ERROR}`，`SKIPPED` 不计入。真正会误报的是 `ALL_ORDERS_SKIPPED`（`pipeline_monitor.py:171-189`，**CRIT**）：它的条件是当日 LIVE 回执全为 `SKIPPED` 且 `filled_qty == 0`，而一个三只票全额抵销的交易日恰好就是这个形状——抵销越成功，告警越响。该规则必须改成「`netted_qty > 0` 的回执算作股数已变动」。`_oversold_codes` 不受影响（它按 `filled_qty` 累计，抵销腿恒为 0）。

...
# ── token：复用 tushare collector 的模块级配置 ────────────────────────────────

_TUSHARE_DIR = Path(__file__).resolve().parent.parent / "tushare"


def _get_pro():
    spec = importlib.util.spec_from_file_location("_ts_conf", _TUSHARE_DIR / "collector.py")
    mod = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(mod)
    except Exception:
        pass  # 只需其 os.environ["TUSHARE_TOKEN"] 赋值生效

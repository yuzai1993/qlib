...
| 每日信号 / 实盘计划 | 使用 **Qlib + `live_trading`** |
...
本仓库的 `live_trading.modules.code_map` 统一维护 `instrument` ↔ `ts_code` 映射；QMT 格式与 Tushare 格式相同。
...
4. **先用 SIMULATE 跑通全链路**，再切真实资金；运行模式与实盘双开关保持显式。

日频调仓流程使用 T-1 信号，并与指定 parity backtest 保持同一决策口径：
...
- [ ] 成交回写 Live SQLite，保持持仓、现金、回执单一事实源

...
## 6. 补充讨论：首日 adjclose 标准化

当前数据管道在前复权之后，还会将所有价格除以首日 adjclose 进行标准化。这是一个**独立于本次修改的问题**，但值得记录：

**现状**：标准化后，每只股票的首日前复权收盘价 = 1.0，价格不再是真实人民币价格。

**影响**：
- Alpha158 特征几乎全是比值或相对量，标准化因子会约掉，**不影响特征值**
- Label 是相邻日 close 的比值，标准化因子约掉，**不影响 Label**
- 回测中，不同股票的价格尺度不同（因标准化因子不同），但 TopK 等等权策略不受影响

**潜在问题**：
- 涉及绝对金额的策略（如最小交易金额限制）会失真
- 跨股票的价格比较无意义
- 与真实市场价格脱节，不利于交易模拟的直觉理解

**建议**：本次不修改首日标准化逻辑，作为后续独立议题评估。若未来需要移除，只需将 `_manual_adj_data()` 改为恒等变换即可。

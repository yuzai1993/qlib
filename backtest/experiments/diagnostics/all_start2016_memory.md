...
各次系统统计均为 0 swaps。seed 42 全流程约 580 秒，其中：
...
1. 当前可直接采用：全 A 从 2016 开始、五种子严格串行；本次已验证。
...
补充：本地 `all` universe 包含 4 个 `SH000`/`SZ399` 风格指数代码，严格意义上
不是纯 A 股股票池。该问题不影响本次 OOM 可运行性结论，但后续若采纳全 A 训练，
应另建过滤后的纯股票 universe 再做模型实验。

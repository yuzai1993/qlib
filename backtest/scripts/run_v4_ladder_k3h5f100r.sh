#!/bin/bash
# v4 官方合成信号 × 真阶梯 k3h5 + f100 补新票。只回测一次，不晋升执行层基线。
set -euo pipefail
...
LOG=backtest/result/m0h20rankices_ladder_k3h5f100r.log
mkdir -p "$PRED_DIR"

echo "===== V4-LADDER-K3H5F100R-START $(date '+%F %H:%M') =====" | tee -a "$LOG"
...
$PY backtest/scripts/run_regime_phase_s.py \
  --arms m0h20rankices --pool all --strategy ladder_k3h5f100r \
...
echo "===== V4-LADDER-K3H5F100R-DONE $(date '+%F %H:%M') =====" | tee -a "$LOG"

...
回执只反映策略「以为」成交了什么，漏记就无人发现。所以桥接在批次终结时（14:45~14:57）额外向 `outbound/` 写一份券商自己的口径，并在 15:01 后用收盘后的账户状态覆盖重写（终结时那份只是 QMT 提前关闭时的兜底，盘中市值/资金口径可能不准）：
...
**模拟盘例外**：QMT 模拟账户的可用资金/市值口径不可信（与界面显示都对不上），现金以本地账本为准。配置 `monitor.broker_reconcile.cash_check: false` 可关闭两条现金类告警（`CASH_NEGATIVE` / `BROKER_CASH_MISMATCH`），持仓对账不受影响。**切换到真实账户后务必改回 `true`**。

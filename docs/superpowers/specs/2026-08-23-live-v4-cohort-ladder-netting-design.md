...
**成本提示**。handler 从 CSI1000（约 1000 只）扩到全A（约 5400 只），内存与耗时都会显著上升。
实施时必须实测一次完整 `--dry-run` 的墙钟时间与峰值内存，再据此调整 16:00 发布 cron 的
时间预算；跑不完就要改成预构建特征帧缓存。

...
3. CLI 极简：只传 `--config`；`note` / `n_runs` / `from_session` / 日期等全部进 YAML（改测试区间只改 `segments.test`）。
...
- 拆开「信号区间」与「组合回测区间」（二者始终由 `segments.test` 对齐）。
...
segments:
  ...
  test:  ["2023-09-18", "2026-03-10"]   # 改测试/回测区间只改这里
...
backtest:
  account: 1000000              # start/end 由 segments.test 自动对齐
  exchange_kwargs: { ... }
```
...
| test / backtest 日期 | `segments.test`；`backtest.start/end` 加载时强制对齐 | 同左（用本 YAML 的 `segments.test`，不是源 session 原区间） |
| 模型文件 | 新训 | `from_session` → `run_XX/mlruns_link.json` → `{train_artifacts}/artifacts/trained_model` 直接反序列化（不依赖 mlruns 注册表按实验名查找） |

**日期对齐规则（两种模式通用）**：以 `segments.test` 为准——

- `backtest.start_time/end_time` 始终设为 `segments.test`；
- 若 `segments.test[1]` 晚于 `data.handler.end_time`，自动将 handler `end_time` 延长；
- handler `start_time` **保持 YAML 原值，不得收窄**到 test 起点（Alpha158 滚动窗口最长 60 日，需要测试区间之前的历史数据）。
...
`--config` 可为绝对/相对路径，或相对 `backtest/configs/` 的文件名；省略则默认 `csi300_lgbm.yaml`。`run.mode` 缺省时默认 `train_backtest`。旧字段 `run.test_start` / `test_end` 若仍存在会被忽略并丢弃。

## Dual Mode Behavior

### Mode `train_backtest`

1. 加载 YAML，按 `segments.test` 对齐 backtest / handler.end_time。
2. 训练 → 回测 → 报告归档（与现逻辑一致）。
3. 支持 `n_runs > 1`。
...
- `overrides`（实际生效的 `segments.test` 等）
- `config_path`
...
| `backtest/scripts/config_loader.py` | 新增：加载、解析路径、按 `segments.test` 对齐日期、校验 `run` |
...
```yaml
# 继承同一套策略/数据描述；改 run.from_session 与 segments.test 即可
run:
  mode: backtest_only
  note: "bt_only_2024起"
...
data: { ... }
segments:
  test: ["2024-01-01", "2026-03-10"]   # 只改这里
model: { ... }
strategy: { ... }
backtest: { ... }   # start/end 由 segments.test 自动对齐
```

## Testing

- 加载默认 yaml，断言关键字段存在且与旧硬编码一致。
- `segments.test` 对齐后 `backtest` 日期一致；终点超出 handler `end_time` 时自动延长；handler `start_time` 不被改动。
- 旧字段 `run.test_start/end` 若存在则忽略并丢弃。
